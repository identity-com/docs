...
`AccountArgument` is the main type used to create the lists of accounts `cruiser` programs use, but we can also create custom types for reuse by our programs or for other programs. 

## Simple Derive

The simplest way to do this is by deriving `AccountArgument` the same way we would for an instruction's accounts:
...
Then we can use this in other account argument derivations just like cruiser's built in types:
...
We can also require data to be passed in and use it like any other type:
...
## Derive With Multiple Data Options

If we want there to be multiple paths to build or verify the argument we can use the `id` attribute:
...
## SingleIndexable and MultiIndexable

If we have a type that it makes sense to be able to operate on a subset of the accounts or on individual accounts we can implement the `SingleIndexable` and `MultiIndexable` traits. These traits are what powers the `signer`, `writable`, `owner`, and `key` validation attributes. `key` uses `SingleIndexable` while the rest use `MultiIndexable`. If you implement `SingleIndexable<()>` that means your argument is a wrapper around a single account.
...
If we want to use it we can do so like this:
...
If we want some custom logic that may not be possible to express with the `AccountArgument` derive, then we manually implement the traits. For this example we will have a set of accounts that all need to point to each other in a ring.
